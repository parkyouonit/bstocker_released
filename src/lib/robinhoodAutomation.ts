...
export async function deployRobinhoodAutomationVault(account: Address, bootstrap: RobinhoodAutomationBootstrap) {
  ...
  if (version !== '3.0.0' || [owner, recipient, guardian].some(value => getAddress(value) !== getAddress(account)) || getAddress(keeper) !== bootstrap.keeperAddress) {
...
export async function setRobinhoodAutomationAuthorization(account: Address, executorAddress: Address, armed: boolean) {
  ...
  try {
    ...
    // 공개 터널이 설정 저장 후 응답 단계에서 5xx를 반환할 수 있다. 동일 서명을
...
function friendlyDepositError(error: unknown, action: '시작' | '추가 입금') {
  ...
  if (message.includes('InvalidMode')) return new Error(action === '추가 입금' ? '이 Vault 버전에서는 운용 중 추가 입금을 지원하지 않습니다.' : '새 포지션 시작은 PAUSED 또는 완전히 회수된 상태에서만 가능합니다.')
  if (message.includes('EmergencySwapIncomplete')) return new Error('MEV 가격 한계 안에서 SPCX 전량을 USDG로 바꿀 수 없어 안전 종료 전체를 되돌렸습니다. 자산은 기존 LP/Vault 상태로 보존됩니다.')
  ...
  if (message.includes('PSC')) return new Error('민트 비율이 Slipstream 가격 검사(PSC)를 통과하지 못했습니다. 새로고침 후 다시 시도하세요. 자금은 Vault로 이동하지 않았습니다.')
...
export type RobinhoodVaultOwnerAction = 'pause' | 'resume' | 'resetAfterExit' | 'withdrawToIdle' | 'exitToTokens' | 'exitToUsdgAuto'
...
export async function executeRobinhoodVaultOwnerAction(account: Address, executorAddress: Address, action: RobinhoodVaultOwnerAction): Promise<Hash> {
  ...
  const data = deadlineActions.includes(action as typeof deadlineActions[number])
    ...
    : encodeFunctionData({ abi: vaultAbi, functionName: action as 'pause' | 'resume' | 'resetAfterExit', args: [] })
