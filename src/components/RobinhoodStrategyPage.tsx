...
export function RobinhoodStrategyPage({ data, loading, refreshing, error, walletAddress, onConnect, onRefresh, onBack, oracleTransaction, onPrepareOracle }: RobinhoodStrategyPageProps) {
  ...
  const currentPilotLimit = data.automation.vault?.maxPilotUsdg ?? data.guardConfig.pilotCapitalUsd
  ...
  return (
    <div className="strategy-page">
      <header className="strategy-header">
        ...
        <div className="strategy-header-actions"><span className="strategy-network">RH · 4663</span><button type="button" onClick={onConnect}>{walletAddress ? shortAddress(walletAddress) : 'Rabby 연결'}</button><button type="button" onClick={onRefresh} disabled={refreshing}>{refreshing ? '…' : '↻'}</button></div>
      </header>
      ...
      <main className="strategy-content">
        <section className="strategy-metrics">
          ...
          <article><span>ROBINHOOD MID</span><strong>{official?.tokenPrice == null ? '—' : formatNumber(official.tokenPrice, 4)}</strong><small>{official?.generatedAt ? new Date(official.generatedAt).toLocaleTimeString('ko-KR') : '가격 없음'}</small></article>
        ...
        <section className="strategy-grid">
          ...
          <aside className="strategy-side-column">
            <article className="strategy-guard-card">
              ...
              <dl><div><dt>1분 Soft Pause</dt><dd>{data.guardConfig.softDrop1mPercent}%</dd></div><div><dt>5분 Withdraw</dt><dd>{data.guardConfig.withdrawDrop5mPercent}%</dd></div><div><dt>5분 USDG Exit</dt><dd>{data.guardConfig.exitDrop5mPercent}%</dd></div><div><dt>공식가 신선도</dt><dd>{data.guardConfig.officialMaxAgeSec}초</dd></div><div><dt>최대 Exit 충격</dt><dd>{data.guardConfig.maxExitPriceImpactPercent}%</dd></div><div><dt>10분 재배치</dt><dd>최대 {data.guardConfig.maxRebalances10m}회</dd></div><div><dt>일 손실 Soft Stop</dt><dd>{data.guardConfig.navSoftLossPercent}%</dd></div><div><dt>전략 Hard Stop</dt><dd>{data.guardConfig.navHardLossPercent}%</dd></div></dl>
              <div className={`strategy-oracle-prep ${decision.metrics.onchainTwapReady ? 'ready' : ''}`}><div><span>ONCHAIN TWAP BUFFER</span><strong>{snapshot.observationCardinality} / {snapshot.observationCardinalityNext}</strong><small>{decision.metrics.onchainTwapReady ? '30초·5분 TWAP 사용 가능' : snapshot.observationCardinalityNext >= 64 ? '용량 확장 완료 · 거래 관찰 기록 대기' : 'Rabby 1회 서명으로 64개까지 확장 필요'}</small></div><button type="button" disabled={snapshot.observationCardinalityNext >= 64 || ['simulating', 'pending'].includes(oracleTransaction.status)} onClick={onPrepareOracle}>{oracleTransaction.status === 'pending' ? '확인 중…' : snapshot.observationCardinalityNext >= 64 ? '준비됨' : '오라클 준비'}</button></div>
        ...
        <footer className="strategy-footer">현재 계약 한도 {formatMoney(currentPilotLimit, 0)} · v2.5 한도 $350 · -3%/NAV hard stop은 원물 대기까지 자동 · 최종 USDG 전환은 Rabby owner 확인</footer>
      </main>
    </div>
  )
}
