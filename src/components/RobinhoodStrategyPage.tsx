      ...
      <main className="strategy-content">
        ...
        <section className="strategy-grid">
          ...
          <aside className="strategy-side-column">
            ...
            <article className="strategy-deploy-card"><span>MAINNET EXECUTOR</span><strong>{data.deployment.contractDeployed ? `${data.writesEnabled ? 'LIVE' : '배포됨'} · ${data.executorAddress ? shortAddress(data.executorAddress) : ''}` : data.deployment.contractCompiled ? 'V2.8 컴파일 완료 · 배포 대기' : '컴파일 전'}</strong><p>{data.deployment.note}</p><button type="button" onClick={() => document.querySelector('.strategy-automation')?.scrollIntoView({ behavior: 'smooth' })}>자동화 설정으로 이동</button><small>고정 수령 주소 · {capitalUnlimited ? '온체인 금액 상한 없음' : `현재 계약 ${formatMoney(currentPilotLimit, 0)} 한도`} · {data.automation.vault?.autoUsdgSafetyExit ? '자동 USDG 안전 종료 적용' : 'v2.8 교체 후 자동 USDG 종료 적용'}</small></article>
          </aside>
        </section>
        ...
        <footer className="strategy-footer">{capitalUnlimited ? 'v2.8 온체인 금액 상한 없음' : `현재 계약 한도 ${formatMoney(currentPilotLimit, 0)}`} · {data.automation.vault?.autoUsdgSafetyExit ? '-3% 급락/-5% NAV 자동 USDG 종료 · TWAP·가격한도·직접 Sequencer MEV 방어' : 'v2.8 교체 전 자동 USDG 종료·신규 MEV 가드 미적용'}</footer>
      </main>
    </div>
  )
}
