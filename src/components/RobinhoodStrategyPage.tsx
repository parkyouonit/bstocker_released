      ...
      <main className="strategy-content">
        ...
        <section className="strategy-grid">
          ...
          <aside className="strategy-side-column">
            ...
            <article className="strategy-deploy-card"><span>MAINNET EXECUTOR</span><strong>{data.deployment.contractDeployed ? `${data.writesEnabled ? 'LIVE' : '배포됨'} · ${data.executorAddress ? shortAddress(data.executorAddress) : ''}` : data.deployment.contractCompiled ? 'V2.7 컴파일 완료 · 배포 대기' : '컴파일 전'}</strong><p>{data.deployment.note}</p><button type="button" onClick={() => document.querySelector('.strategy-automation')?.scrollIntoView({ behavior: 'smooth' })}>자동화 설정으로 이동</button><small>고정 수령 주소 · {capitalUnlimited ? '온체인 금액 상한 없음' : `현재 계약 ${formatMoney(currentPilotLimit, 0)} 한도`} · 최종 USDG 전환은 owner 전용</small></article>
          </aside>
        </section>
        ...
        <footer className="strategy-footer">{capitalUnlimited ? 'v2.7 온체인 금액 상한 없음' : `현재 계약 한도 ${formatMoney(currentPilotLimit, 0)}`} · 입력액만 정확히 승인 · -3%/NAV hard stop은 원물 대기까지 자동 · 최종 USDG 전환은 owner 지갑 확인</footer>
      </main>
    </div>
  )
}
